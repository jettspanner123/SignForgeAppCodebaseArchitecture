...
/**
 * Application URL Encoding and Candidate Link Generator Utility Singleton.
 */
export default class ApplicationUrlEncoderUtility {
  public static current: ApplicationUrlEncoderUtility = new ApplicationUrlEncoderUtility();

  /**
   * Encodes essential fields of an OfferDocument into a compact base64 URL parameter.
   */
  public encodeOfferToUrl(doc: OfferDocument): string {
    try {
   ...
   */
  public decodeOfferFromUrl(encodedStr: string): OfferDocument | null {
  ...
  /**
   * Generates a direct candidate eSign URL containing embedded compact offer payload.
   */
  public getCandidateShareLink(doc: OfferDocument): string {
    let origin = '';
    ...
    const payload = this.encodeOfferToUrl(doc);
